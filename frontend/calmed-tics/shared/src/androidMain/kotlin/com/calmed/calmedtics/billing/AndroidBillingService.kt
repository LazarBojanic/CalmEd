...
class AndroidBillingService(
    ...
    private val log = AppLog(LogTags.BILLING)
                ...
                billingClient.startConnection(object : BillingClientStateListener {
                    override fun onBillingSetupFinished(result: BillingResult) {
                        if (result.responseCode == BillingClient.BillingResponseCode.OK) {
                            log.debug("Billing client connected successfully.")
                            if (cont.isActive) cont.resume(Unit)
                        } else {
                            log.error("Billing setup failed (${result.responseCode}): ${result.debugMessage}")
                            if (cont.isActive) {
    ...
    override suspend fun purchase(productId: String, obfuscatedAccountId: String?) {
        ...
        if (launchResult.responseCode != BillingClient.BillingResponseCode.OK) {
            log.error("Failed to launch billing flow: ${launchResult.debugMessage} (code ${launchResult.responseCode})")
    ...
    override suspend fun restore() {
        ...
        log.debug("Restoring purchases, found ${restored.size} owned purchase(s).")
        if (restored.isEmpty()) {
        ...
        when (result.responseCode) {
            BillingClient.BillingResponseCode.OK -> {
                if (!purchases.isNullOrEmpty()) {
                    ...
                    log.debug("Purchases updated with OK, but purchase list was empty.")
                }
            }
            BillingClient.BillingResponseCode.USER_CANCELED -> {
                log.debug("User cancelled the purchase flow.")
                _purchaseResults.tryEmit(PurchaseResult.Failure("User cancelled"))
            }
            BillingClient.BillingResponseCode.ITEM_ALREADY_OWNED -> {
                log.debug("Item already owned. Attempting to restore purchases.")
                _purchaseResults.tryEmit(PurchaseResult.Failure("Item is already owned."))
            }
            else -> {
                log.error("Billing error on purchases updated (${result.responseCode}): ${result.debugMessage}")
    ...
    override suspend fun productPrice(productId: String): String? {
        val details = productCache[productId] ?: fetchProductDetails(productId) ?: return null
    ...
    private suspend fun fetchProductDetails(productId: String): ProductDetails? {
        ...
        if (found == null) {
            log.debug("Product '$productId' not found as INAPP, trying SUBS...")
            found = tryQuery(BillingClient.ProductType.SUBS)
        }

        if (found != null) {
            log.debug("Product '$productId' found and cached.")
            productCache[productId] = found
        } else {
            log.warn("Product '$productId' not found in any category (INAPP / SUBS).")
        }
        return found
    }
    ...
    private fun processPurchase(purchase: Purchase) {
        if (purchase.purchaseState != Purchase.PurchaseState.PURCHASED) {
            log.debug("Purchase state is ${purchase.purchaseState}, skipping acknowledgement until purchased.")
            return
        }
        ...
        suspendCancellableCoroutine { cont ->
            billingClient.acknowledgePurchase(params) { billingResult ->
                if (billingResult.responseCode == BillingClient.BillingResponseCode.OK) {
                    log.debug("Purchase acknowledged after server verification.")
                } else {
                    log.error(
