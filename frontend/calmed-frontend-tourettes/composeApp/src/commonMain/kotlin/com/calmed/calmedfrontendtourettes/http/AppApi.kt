...
class AppApi(private val appHttpClient: AppHttpClient, private val tokenDataStore: ITokenDataStore
	...
	override suspend fun getUserInfoTourettesByUserId(userId: String): UserInfoTourettesDto? {
		val resp: HttpResponse = client.get("/user-info-tourettes/user/$userId")
		return when (resp.status) {
			HttpStatusCode.OK -> resp.body<UserInfoTourettesDto>()
			else -> null
		}
	}

	override suspend fun updateUserInfoTourettes(id: String, dto: UserInfoTourettesUpdateDto
	): UserInfoTourettesDto? {
		val resp: HttpResponse = client.put("/user-info-tourettes/$id") { setBody(dto) }
		return when (resp.status) {
			HttpStatusCode.OK -> resp.body<UserInfoTourettesDto>()
			else -> null
		}
